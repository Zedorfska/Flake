{ self, ... }: {
  ...
  in {
    options.device.features.environments.dwl.enable =
      lib.mkEnableOption "DWL tiling Wayland compositor";
    ...
    packages.dwl =
      let
        ...
        configH = pkgs.writeText "config.h" ''
          ...
          static const float focuscolor[]            = COLOR(0x7c7cffff);
          ...
          static const MonitorRule monrules[] = {
            ...
            { "HDMI-A-1", 0.55f, 1, 1, &layouts[0], WL_OUTPUT_TRANSFORM_NORMAL, 1920, 15 },
          ...
          static const Key keys[] = {
            /* --- apps (from twm.defaults) --- */
            { MODKEY,                    XKB_KEY_c,      spawn,            CMD("${terminal}") },
            { MODKEY,                    XKB_KEY_r,      spawn,            SHCMD("${menu}") },
            { MODKEY,                    XKB_KEY_e,      spawn,            CMD("${fileManager}") },
